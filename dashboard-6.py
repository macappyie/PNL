...
if uploaded_file:
    try:
        ...
        if 'Txn Date' not in df or 'Credit' not in df or 'Debit' not in df:
            ...
            df = df.dropna(subset=['Txn Date', 'Credit', 'Debit'])
            ...
            # Debug June check
            #june_check = filtered_df[(filtered_df['Txn Date'].dt.month == 6)]
            #st.write("🛠️ Month data preview:", june_check)

            # Group by Month
            monthly_pnl = filtered_df.groupby('Month_dt')['Net P&L'].sum().reset_index()
